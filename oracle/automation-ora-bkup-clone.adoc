...
summary: '该解决方案提供了一个基于 Ansible 的自动化工具包，用于从驻留在NetApp云卷中的 Oracle 数据库映像备份副本设置、克隆和刷新 Oracle 克隆的超大型数据库 (VLDB)，这些副本可从公共云平台获得。该工具包最初支持Google Cloud NetApp Volumes (GCNV)，但可以扩展以支持其他云卷，例如Azure NetApp Files (ANF) 或Amazon FSx for ONTAP (FSxN)。' 
---
...
[role="lead"]
该解决方案提供了一个基于 Ansible 的自动化工具包，用于从驻留在NetApp云卷中的 Oracle 数据库映像备份副本设置、克隆和刷新 Oracle 克隆的超大型数据库 (VLDB)，这些副本可从公共云平台获得。该工具包最初支持Google Cloud NetApp Volumes (GCNV)，但可以扩展以支持其他云卷，例如Azure NetApp Files (ANF) 或Amazon FSx for ONTAP (FSxN)。



== 目的

利用公共云中的服务器内数据库映像备份副本为 Oracle VLDB 提供NetApp ONTAP卷的快速克隆（几分钟）功能可实现多种目的。它为报告、开发或 UAT 用例提供了近乎实时的全尺寸生产数据库。它可以节省存储成本，特别是当可以选择对主数据库备份卷进行精简克隆时。这个基于 Ansible 的自动化工具包使用户能够按照用户希望的时间表设置、克隆和刷新克隆的 Oracle 数据库，以简化生命周期管理。该工具包适用于部署到 Google 公共云的 Oracle VLDB 用例，并使用 GCNV 作为服务器内数据库备份存储。
...
[%collapsible%open]
====
该工具包包含一个 hosts 文件，该文件定义了 Ansible playbook 运行的源 Oracle 主机和目标 Oracle 主机。通常包括主数据库服务器主机和目标克隆数据库服务器主机。以下是示例文件。主机条目包括目标主机 IP 地址以及 ssh 密钥，以便用户访问主机执行克隆或刷新命令。通过 gcloud cli 访问和管理Google Cloud NetApp Volumes存储。
...
[%collapsible%open]
====
主机变量在 host_vars 目录中定义，名为 {{ host_name }}.yml，仅适用于特定主机。对于此解决方案，仅配置目标克隆DB主机参数文件。 Oracle 主数据库参数在全局 vars 文件中配置。下面是目标 Oracle 克隆 DB 主机变量文件 orac.yml 的示例，显示了典型配置。
...
[%collapsible%open]
====
克隆目标 Oracle 服务器应安装并修补与源 Oracle 服务器相同的 Oracle 软件堆栈。 $ORACLE_HOME 变量理想情况下应该与源 Oracle 服务器设置相匹配。如果目标 ORACLE_HOME 设置与主 Oracle 服务器配置不同，请创建一个符号链接来解决这些差异。
...
要克隆任何其他数据库，请创建单独的 oracle_bkup_clone_n_gcnv.yml 和 oracle_bkup_clone_n_gcnv.sh。相应地配置 Ansible 目标主机、全局 vars.yml 和 host_vars 目录中的 hostname.yml 文件。
